...
// Usamos POOL porque permite trabajar correctamente
// con varias consultas y transacciones.
    ...
    try {
        ...
        for (const item of detalles) {
            ...
            if (productos[0].stock_actual < cantidad) {
                ...
                throw new Error(
                    `Stock insuficiente para "${productos[0].nombre}".`
                );
            }
        }
...
app.get("/api/ventas/:id", async (req, res) => {
    ...
    try {
        ...
        // -------------------------------------------------
        // PRIMERO VERIFICAMOS QUE LA VENTA EXISTA
        // -------------------------------------------------
...
app.put("/api/ventas/:id", async (req, res) => {
    ...
    try {
        ...
        for (const item of detalles) {
            ...
            if (
                productos[0].stock_actual < cantidad
            ) {
...
app.put("/api/ventas/:id/pagar", async (req, res) => {
    ...
    try {
        ...
        if (resultado.affectedRows === 0) {
            ...
            return res.status(404).json({
                error: "Venta fiada no encontrada"
            });
        }

        res.json({
            mensaje:
                "Venta marcada como pagada correctamente"
        });
