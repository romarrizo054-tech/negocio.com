...
function Inventario() {
  ...
  const obtenerCategorias = async () => {
    try {
      const res = await axios.get('https://negocio-com-1.onrender.com/api/categorias');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://negocio-com-1.onrender.com/api/productos', nuevoProducto);
  ...
  const eliminarProducto = async (id) => {
    if (window.confirm('¿Estás segura de que deseas eliminar este producto?')) {
      try {
        await axios.delete(`https://negocio-com-1.onrender.com/api/productos/${id}`);
